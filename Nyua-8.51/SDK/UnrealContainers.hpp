...
{	
	...
	public:
		static void Free(void* Ptr)
		{
            ...
            static void (*FMemoryFree)(void* Ptr) = decltype(FMemoryFree)(uintptr_t(GetModuleHandle(0)) + 0x01BFA7D0); 
		...
		static void* Realloc(void* Ptr, uint64 Size, uint32 Alignment)
		{
            ...
            static void* (*FMemoryRealloc)(void* Ptr, uint64 Size, uint32 Alignment) = decltype(FMemoryRealloc)(uintptr_t(GetModuleHandle(0)) + 0x01C08880); 		
	...
	public:
		...
		int32 AddDefaulted()
		{
			int32 Index = NumElements; // or Data.NumAllocated() if you prefer
			Add(ArrayElementType()); // Add default-constructed element
			return Index;
		}
		inline void Clear()
		{
	...
	*/
	...
	public:
		...
		inline int32 Add(T&& Element)
		{
			...
			if (NumFreeIndices > 0)
			{
				...
				const auto& FreeLink = Data[Index];
				FirstFreeIndex = FreeLink.NextFreeIndex;
				--NumFreeIndices;
			}
			else
			{
			...
			// Placement new the element into the aligned storage

			new (&Data[Index].ElementData) SparseArrayElementType(std::forward<T>(Element));

			// Mark allocated
			AllocationFlags[Index] = true;//[Index] = true;
	...
	public:
		...
		inline bool operator!=(const TSet<SetElementType>& Other) const { return Elements != Other.Elements; }
	public:
		inline void Add(SetElementType& Element)
		{
			// Check for duplicate
			for (int32 Index = 0; Index < Elements.Num(); ++Index)
			{
				...
				{
					if (Elements[Index].Value == Element)
					{
			...
			// (Optional) Update hash if needed
			// Not implemented yet
		}
	public:
